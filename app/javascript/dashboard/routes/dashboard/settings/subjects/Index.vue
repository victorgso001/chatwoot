  ...
  <div class="flex-1 overflow-auto p-4">
    ...
    <div class="flex flex-row gap-4">
      <div class="w-[60%]">
        ...
        >
          {{ $t('SUBJECT_MGMT.LIST.404') }}
        </p>
        <woot-loading-state
          v-if="uiFlags.isFetching"
          :message="$t('SUBJECT_MGMT.LOADING')"
        />
        <table v-if="!uiFlags.isFetching && records.length" class="woot-table">
          <thead>
            <th
              v-for="thHeader in $t('SUBJECT_MGMT.LIST.TABLE_HEADER')"
              :key="thHeader"
            >
              {{ thHeader }}
            </th>
          ...
          <tbody>
            <tr v-for="(label, index) in records" :key="label.title">
              ...
              <td class="button-wrapper">
                <woot-button
                  v-tooltip.top="$t('SUBJECT_MGMT.FORM.EDIT')"
                  variant="smooth"
                  size="tiny"
                ...
                />
                <woot-button
                  v-tooltip.top="$t('SUBJECT_MGMT.FORM.DELETE')"
                  variant="smooth"
                  color-scheme="alert"
                  size="tiny"
                  icon="dismiss-circle"
      ...
      <div class="w-[34%]">
        <span v-dompurify-html="$t('SUBJECT_MGMT.SIDEBAR_TXT')" />
      </div>
    </div>
    ...
    <woot-delete-modal
      ...
      :title="$t('SUBJECT_MGMT.DELETE.CONFIRM.TITLE')"
      :message="$t('SUBJECT_MGMT.DELETE.CONFIRM.MESSAGE')"
